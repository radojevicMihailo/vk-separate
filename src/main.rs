...
use nova_snark::{
    nova::VerifierKey,
    provider::{pasta::pallas::Scalar as PallasScalar, PallasEngine, VestaEngine},
    traits::{
...
#[command(
    name = "vk_separate",
    version = "1.0",
    about = "Processes JSON files by removing 'ck' fields."
)]
struct Cli {
    /// Input JSON file
    input_file: String,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn create_bin_file<E1, E2, C1, EE1, EE2>(
        path: &str,
        file_name: &str,
        ...
        EE2: EvaluationEngineTrait<E2>,
    {
        ...
        // ! From string into Vk
        let json_data_vk: VerifierKey<E1, E2, C1, S<E1, EE1>, S<E2, EE2>> =
    ...
    let _ = create_bin_file::<PallasEngine, VestaEngine, TrivialCircuit<PallasScalar>, EE<_>, EE<_>>(
        &args.output_dir,
        &args.output_file,
    )?;
